...
public class ClickableKeybinding extends ClickableButton {
            ...
            if ((player.worldObj.isRemote) && Config.toggleModuleSpam()) {
                player.addChatMessage(
                    new ChatComponentText(
                        "Toggled " + module.getModule()
                            .getDataName() + valstring));
            }
            MuseItemUtils.toggleModuleForPlayer(
                player,
                module.getModule()
                    .getDataName(),
                toggleval);
            MusePacketToggleRequest toggleRequest = new MusePacketToggleRequest(
                player,
                module.getModule()
                    .getDataName(),
                toggleval);
        ...
        for (ClickableModule module : boundModules) {
            ...
            if (displayOnHUD) {
                MuseRenderer.drawString(
                    MuseStringUtils.wrapFormatTags("HUD", MuseStringUtils.FormatCodes.BrightGreen),
                    this.position.x() * 2 + 6,
                    this.position.y() * 2 + 6);
            } else {
                MuseRenderer.drawString(
                    MuseStringUtils.wrapFormatTags("x", MuseStringUtils.FormatCodes.Red),
                    this.position.x() * 2 + 6,
                    this.position.y() * 2 + 6);
            }
            GL11.glPopMatrix();
    ...
    public void unbindFarModules() {
        ...
        while (iterator.hasNext()) {
            ...
            double distanceSq = module.getPosition()
                .distanceSq(this.getPosition());
    ...
    public void attractBoundModules(IClickable exception) {
        for (ClickableModule module : boundModules) {
            if (!module.equals(exception)) {
                MusePoint2D euclideanDistance = module.getPosition()
                    .minus(this.getPosition());
                MusePoint2D directionVector = euclideanDistance.normalize();
                MusePoint2D tangentTarget = directionVector.times(getTargetDistance())
                    .plus(this.getPosition());
                MusePoint2D midpointTangent = module.getPosition()
