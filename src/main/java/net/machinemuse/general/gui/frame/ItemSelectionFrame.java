...
public class ItemSelectionFrame extends ScrollableFrame {
    ...
    public ItemSelectionFrame(MusePoint2D topleft, MusePoint2D bottomright, Colour borderColour, Colour insideColour,
        EntityPlayer player) {
    ...
    private void drawSelection() {
        if (selectedItemStack != -1) {
            MuseRenderer.drawCircleAround(
                Math.floor(
                    itemButtons.get(selectedItemStack)
                        .getPosition()
                        .x()),
                Math.floor(
                    itemButtons.get(selectedItemStack)
    ...
    public List<String> getToolTip(int x, int y) {
        ...
        if (itemHover > -1) {
            return itemButtons.get(itemHover)
                .getToolTip();
        } else {
            return null;
        }
