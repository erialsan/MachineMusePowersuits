...
public class PartManipSubFrame {
    ...
    public PartManipSubFrame(ModelSpec model, ColourPickerFrame colourframe, ItemSelectionFrame itemSelector,
         ...
         * var specs: Array[ModelPartSpec] = model.apply.values.filter(spec => isValidArmor(getSelectedItem,
         * spec.slot)).toArray
    ...
    private List<ModelPartSpec> getSpecs() {
        ...
        Iterator<ModelPartSpec> specIt = model.apply()
            .values()
            .iterator();
        ModelPartSpec spec;
        while (specIt.hasNext()) {
            spec = specIt.next();
            if (isValidArmor(getSelectedItem(), spec.slot)) specsArray.add(spec);
        }
        return specsArray;
    }

    public int getArmorSlot() {
        return ((ItemArmor) this.getSelectedItem()
            .getItem()
            .getItem()).armorType;
    }
    ...
    public NBTTagCompound getRenderTag() {
        return MuseItemUtils.getMuseRenderTag(
    ...
    public NBTTagCompound getItemTag() {
        return MuseItemUtils.getMuseItemTag(
            this.getSelectedItem()
                .getItem());
    }

    public boolean isValidArmor(final ClickableItem clickie, final int slot) {
        return clickie != null && clickie.getItem()
            .getItem()
            .isValidArmor(clickie.getItem(), slot, (Entity) Minecraft.getMinecraft().thePlayer);
    }
    ...
    public NBTTagCompound getOrDontGetSpecTag(final ModelPartSpec spec) {
        final String name = ModelRegistry.getInstance()
            .makeName(spec);
        return this.getRenderTag()
            .hasKey(name)
                ? this.getRenderTag()
    ...
    public void decrAbove(int index) {
        for (ModelPartSpec spec : specs) {
            String tagname = ModelRegistry.getInstance()
                .makeName(spec);
            ...
            if (tagdata != null) {
                ...
                if (oldindex >= index && oldindex > 0) {
                    ...
                    if (player.worldObj.isRemote) PacketSender.sendToServer(
                        new MusePacketCosmeticInfo(player, getSelectedItem().inventorySlot, tagname, tagdata)
                            .getPacket131());
                }
            }
        }
    ...
    public void drawSpecPartial(final double x, final double y, final ModelPartSpec spec, final double ymino,
        final double ymaxo) {
    ...
    public boolean tryMouseClick(final double x, final double y) {
        if (x < this.border.left() || x > this.border.right() || y < this.border.top() || y > this.border.bottom()) {
            return false;
        } else if (x > this.border.left() + 2 && x < this.border.left() + 8
            && y > this.border.top() + 2
            && y < this.border.top() + 8) {
                this.open = !this.open;
                this.getBorder();
                ...
                switch (columnNumber) {
                    case 0: {
                        ...
                        final String tagname3 = ModelRegistry.getInstance()
                            .makeName(spec);
                        ...
                        if (player3.worldObj.isRemote) {
                            PacketSender.sendToServer(
                                new MusePacketCosmeticInfo(
                                    (EntityPlayer) player3,
                                    this.getSelectedItem().inventorySlot,
                                    tagname3,
                                    new NBTTagCompound()).getPacket131());
                        }
                        this.updateItems();
                        return true;
                    }
                    ...
                    case 1: {
                        final String tagname2 = ModelRegistry.getInstance()
                            .makeName(spec);
                    ...
                    case 2: {
                        final String tagname = ModelRegistry.getInstance()
                            .makeName(spec);
            ...
            } else if (x > this.border.left() + 28
                && x < this.border.left() + 28 + this.colourframe.colours().length * 8) {
                    ...
                    final String tagname4 = ModelRegistry.getInstance()
                        .makeName(spec2);
