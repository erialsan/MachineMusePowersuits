...
public class InstallSalvageFrame extends ScrollableFrame {
    ...
    public InstallSalvageFrame(EntityClientPlayerMP player, MusePoint2D topleft, MusePoint2D bottomright,
        Colour borderColour, Colour insideColour, ItemSelectionFrame targetItem, ModuleSelectionFrame targetModule) {
        ...
        this.installButton = new ClickableButton(
            StatCollector.translateToLocal("gui.install"),
            new MusePoint2D(border.right() - sizex / 2.0, border.bottom() - sizey / 4.0),
            true);
        this.salvageButton = new ClickableButton(
            StatCollector.translateToLocal("gui.salvage"),
    ...
    public List<String> getToolTip(int x, int y) {
        if (targetItem.getSelectedItem() != null && targetModule.getSelectedModule() != null) {
            ItemStack stack = targetItem.getSelectedItem()
                .getItem();
            IPowerModule module = targetModule.getSelectedModule()
                .getModule();
            ...
            if (yoffset + 16 > y && yoffset < y) {
                ...
                if (xoffset + 16 * itemsToCheck.size() > x && xoffset < x) {
                    ...
                    return (List<String>) itemsToCheck.get(index)
