...
public class ModuleSelectionFrame extends ScrollableFrame {
    ...
    public ModuleSelectionFrame(MusePoint2D topleft, MusePoint2D bottomright, Colour borderColour, Colour insideColour,
        ItemSelectionFrame target) {
    ...
    private void drawItems() {
        for (ModuleSelectionSubFrame frame : categories.values()) {
            frame.drawPartial(
                (int) (this.currentscrollpixels + border.top() + 4),
    ...
    private void drawSelection() {
        ...
        if (module != null) {
            MusePoint2D pos = moduleButtons.get(selectedModule)
                .getPosition();
            if (pos.y() > this.currentscrollpixels + border.top() + 4
                && pos.y() < this.currentscrollpixels + border.top() + border.height() - 4) {
        ...
        if (selectedItem != null) {
            ...
            if (workingModules.size() > 0) {
                ...
                for (IPowerModule module : workingModules) {
                    ...
                    if (moduleClickable.getModule()
                        .equals(this.prevSelection)) {
    ...
    public List<String> getToolTip(int x, int y) {
        if (border.left() < x && border.right() > x && border.top() < y && border.bottom() > y) {
            ...
            if (moduleButtons != null) {
                ...
                if (moduleHover > -1) {
                    return moduleButtons.get(moduleHover)
                        .getToolTip();
                } else {
                    return null;
                }
