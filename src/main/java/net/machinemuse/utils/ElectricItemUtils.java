...
public class ElectricItemUtils {
    ...
    public static double getPlayerEnergy(EntityPlayer player) {
        double avail = 0.0;
        for (ElectricAdapter adapter : electricItemsEquipped(player)) avail += adapter.getCurrentEnergy();
        return avail;
    }

    public static double getMaxEnergy(EntityPlayer player) {
        double avail = 0.0;
        for (ElectricAdapter adapter : electricItemsEquipped(player)) avail += adapter.getMaxEnergy();
        return avail;
    }
    ...
    public static void drainPlayerEnergy(EntityPlayer player, double drainAmount) {
        ...
        for (ElectricAdapter adapter : electricItemsEquipped(player))
            if (drainleft > 0) drainleft = drainleft - adapter.drainEnergy(drainleft);
            else break;
    }
    ...
    public static void givePlayerEnergy(EntityPlayer player, double joulesToGive) {
        ...
        for (ElectricAdapter adapter : electricItemsEquipped(player))
            if (joulesleft > 0) joulesleft = joulesleft - adapter.giveEnergy(joulesleft);
            else break;
    }
