...
public class MuseItemUtils {
    ...
    public static ItemStack[] itemsEquipped(EntityPlayer player) {
        return new ItemStack[] { player.inventory.armorInventory[0], player.inventory.armorInventory[1],
            player.inventory.armorInventory[2], player.inventory.armorInventory[3], player.inventory.getCurrentItem() };
    }
    ...
    public static boolean canStackTogether(ItemStack stack1, ItemStack stack2) {
        if (!isSameItem(stack1, stack2)) {
            ...
            return false;
        } else return stack1.stackSize < stack1.getMaxStackSize();
    }
    ...
    public static boolean isSameItem(ItemStack stack1, ItemStack stack2) {
        if (stack1 == null || stack2 == null) {
            ...
            return false;
        } else return !((!stack1.isItemStackDamageable()) && (stack1.getItemDamage() != stack2.getItemDamage()));
    }

    public static void transferStackWithChance(ItemStack itemsToGive, ItemStack destinationStack,
    ...
    public static Set<Integer> giveOrDropItemWithChance(ItemStack itemsToGive, EntityPlayer player,
        double chanceOfSuccess) {
    ...
    public static boolean canItemFitInInventory(EntityPlayer player, ItemStack itemstack) {
        ...
        if (!itemstack.isItemDamaged()) {
            ...
            for (int i = 0; i < player.inventory.getSizeInventory(); i++) {
                ...
                if (invstack != null && invstack.getItem() == itemstack.getItem()
                    && invstack.isStackable()
                    && invstack.stackSize < invstack.getMaxStackSize()
                    && invstack.stackSize < player.inventory.getInventoryStackLimit()
                    && (!invstack.getHasSubtypes() || invstack.getItemDamage() == itemstack.getItemDamage())) {
                    return true;
                }
            }
        }
    ...
    public static NBTTagCompound getFluidTermTag(ItemStack stack) {
        ...
        if (stack != null && stack.getItem() instanceof IModularItem) {
            ret = MuseItemUtils.getMuseItemTag(stack)
                .getCompoundTag("AppEng EC Wireless Fluid Terminal");
        }
        return ret;
    }
    ...
    public static boolean getCanShrink(ItemStack stack) {
        if (stack != null && stack.getItem() instanceof IModularItem) {
            ...
            NBTTagCompound cmTag = ((itemTag.hasKey("CompactMachines")) ? itemTag.getCompoundTag("CompactMachines")
                : null);
