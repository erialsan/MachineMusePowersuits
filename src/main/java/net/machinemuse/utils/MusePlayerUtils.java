...
public class MusePlayerUtils {
    ...
    public static MovingObjectPosition raytraceEntities(World world, EntityPlayer player, boolean collisionFlag,
    ...
    public static MovingObjectPosition raytraceBlocks(World world, EntityPlayer player, boolean collisionFlag,
        double reachDistance) {
        Vec3 playerPosition = Vec3.createVectorHelper(player.posX, player.posY + player.getEyeHeight(), player.posZ);
        Vec3 playerLook = player.getLookVec();
    ...
    public static MovingObjectPosition doCustomRayTrace(World world, EntityPlayer player, boolean collisionFlag,
        double reachDistance) {
    ...
    public static double thrust(EntityPlayer player, double thrust, boolean flightControl) {
        ...
        if (flightControl) {
            ...
            desiredDirection.xCoord = desiredDirection.xCoord * Math.signum(forwardkey)
                + strafeX * Math.signum(strafekey);
            desiredDirection.yCoord = flightVerticality * desiredDirection.yCoord * Math.signum(forwardkey)
                + (jumpkey ? 1 : 0)
                - (downkey ? 1 : 0);
            desiredDirection.zCoord = desiredDirection.zCoord * Math.signum(forwardkey)
                + strafeZ * Math.signum(strafekey);
    ...
    public static double getPlayerCoolingBasedOnMaterial(EntityPlayer player) {
        ...
        cool += ((2.0 - getBiome(player).getFloatTemperature((int) player.posX, (int) player.posY, (int) player.posZ))
            / 2); // Algorithm that returns a value from 0.0 -> 1.0. Biome temperature is from 0.0 -> 2.0
        if ((int) player.posY > 128) { // If high in the air, increase cooling
            cool += 0.5;
        }
        if (!player.worldObj.isDaytime() && "Desert".equals(getBiome(player).biomeName)) { // If nighttime and in the
                                                                                           // desert, increase cooling
            cool += 0.8;
        }
    ...
    public static BiomeGenBase getBiome(EntityPlayer player) {
        ...
        return chunk.getBiomeGenForWorldCoords(
            (int) player.posX & 15,
            (int) player.posZ & 15,
            player.worldObj.getWorldChunkManager());
    }
