...
public class IC2ElectricAdapter extends ElectricAdapter {
    ...
    public double drainEnergy(double requested) {
        return ElectricConversions.museEnergyFromEU(
            ElectricItem.manager
                .discharge(stack, ElectricConversions.museEnergyToEU(requested), getTier(), true, false, false));
    }

    @Override
    public double giveEnergy(double provided) {
        return ElectricConversions.museEnergyFromEU(
            ElectricItem.manager.charge(stack, ElectricConversions.museEnergyToEU(provided), getTier(), true, false));
    }
