...
public class Config {
    ...
    public static boolean keybindHUDon() {
        return config.get("HUD", "Display HUD", true)
            .getBoolean();
    }

    public static double keybindHUDx() {
        return config.get("HUD", "x position", 8.0)
            .getDouble();
    }

    public static double keybindHUDy() {
        return config.get("HUD", "y position", 32.0)
            .getDouble();
    }

    public static boolean toggleModuleSpam() {
        return config.get("HUD", "Chat message when toggling modules", false)
            .getBoolean();
    }

    public static double getWeightCapacity() {
        return config.get(Configuration.CATEGORY_GENERAL, "Weight Limit (grams)", 25000.0)
            .getDouble();
    }
    ...
    public static void extractRecipes() {
        ...
        if (!config.hasKey(Configuration.CATEGORY_GENERAL, key)
    ...
    public static void copyRecipe(String inFile) {
        InputStream src = CommonProxy.class.getClassLoader()
    ...
    public static CreativeTabs getCreativeTab() {
        if (mpsCreativeTab == null) mpsCreativeTab = new MPSCreativeTab();
    ...
    public static double getSalvageChance() {
        return config.get(Configuration.CATEGORY_GENERAL, "Salvage Ratio", 0.9)
            .getDouble(0.9);
    }
    ...
    public static double getMaximumArmorPerPiece() {
        return Math.max(
            0.0,
            config.get(Configuration.CATEGORY_GENERAL, "Maximum Armor per Piece", 6.0)
                .getDouble(6.0));
    }

    public static double getMaximumFlyingSpeedmps() {
        return config.get(Configuration.CATEGORY_GENERAL, "Maximum flight speed (in m/s)", 25.0)
            .getDouble(25.0);
    }

    public static boolean useMouseWheel() {
        return config.get(Configuration.CATEGORY_GENERAL, "Use Mousewheel to change modes", true)
            .getBoolean(true);
    }
    ...
    public static double appengMultiplier() {
        return config.get(Configuration.CATEGORY_GENERAL, "Energy per AE", 5.0)
            .getDouble(5.0);
    }

    public static boolean useAdvancedOreScannerMessage() {
        return config.get(Configuration.CATEGORY_GENERAL, "Use Detailed Ore Scanner Message", true)
            .getBoolean(true);
    }

    public static boolean useOldAutoFeeder() {
        return config.get(Configuration.CATEGORY_GENERAL, "Use Old Auto Feeder Method", false)
            .getBoolean(false);
    }

    public static boolean useCheatyLeatherRecipe() {
        return config
            .get(Configuration.CATEGORY_GENERAL, "Use Cheaty Leather Recipe (Requires Thermal Expansion)", true)
            .getBoolean(true);
    }

    public static boolean useHUDStuff() {
        return config
            .get(Configuration.CATEGORY_GENERAL, "Use HUD for certain modules (Auto Feeder, Compass, Clock, etc.", true)
            .getBoolean(true);
    }

    public static boolean use24hClock() {
        return config.get(Configuration.CATEGORY_GENERAL, "Use a 24h clock instead of 12h", false)
            .getBoolean(false);
    }
    ...
    public static String additionalInfoInstructions() {
        ...
        return MuseStringUtils
            .wrapMultipleFormatTags(message, MuseStringUtils.FormatCodes.Grey, MuseStringUtils.FormatCodes.Italic);
    }

    public static boolean useGraphicalMeters() {
        return config.get(Configuration.CATEGORY_GENERAL, "Use Graphical Meters", true)
            .getBoolean(true);
    }

    public static double baseMaxHeat() {
        return config.get(Configuration.CATEGORY_GENERAL, "Base Heat Cap", 50.0)
            .getDouble(50.0);
    }

    public static boolean allowConflictingKeybinds() {
        return config.get(Configuration.CATEGORY_GENERAL, "Allow Conflicting Keybinds", true)
            .getBoolean(true);
    }

    public static boolean useCustomFonts() {
        return config.get("Font", "Use Custom Font Engine", true)
            .getBoolean(true);
    }

    public static double fontDetail() {
        return config.get("Font", "Font Detail Multiplier", 4)
            .getDouble(4);
    }

    public static String fontURI() {
        return config.get("Font", "Font URI", Config.RESOURCE_PREFIX + "fonts/cra.ttf")
            .getString();
    }

    public static String fontName() {
        return config.get("Font", "Native Font Name (Overrides URI)", "")
            .getString();
    }

    public static boolean fontAntiAliasing() {
        return config.get("Font", "Font Anti-Aliasing", false)
            .getBoolean(false);
    }

    public static int glowMultiplier() {
        return config.get("Graphics", "Bloom Multiplier", 3)
            .getInt(3);
    }

    public static boolean useShaders() {
        return config.get("Graphics", "Use Pixel/Vertex Shaders", true)
            .getBoolean(true);
    }
    ...
    public static void addCustomInstallCosts() {
        try {
            ...
            if (installCostFile.exists()) {
                ...
                is.readFully(bytes);
                String string = Charset.defaultCharset()
                ...
                for (InstallCost cost : costs) {
                    ...
                    if (item != null) {
                        ...
                        if (stack != null) {
                            ...
                            MuseLogger.logError(
                                "Invalid Itemstack in custom install cost. Module [" + cost.moduleName
                                    + "], item ["
                                    + cost.itemName
                                    + "]");
                        }
                        ...
                        MuseLogger.logError(
                            "Invalid Item in custom install cost. Module [" + cost.moduleName
                                + "], item ["
                                + cost.itemName
                                + "]");
                    }
