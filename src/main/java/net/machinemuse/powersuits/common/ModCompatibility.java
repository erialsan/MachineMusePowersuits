...
public class ModCompatibility {
    ...
    public static boolean enableThaumGogglesModule() {
        ...
        return Config.getConfig()
            .get("Special Modules", "Thaumcraft Goggles Module", defaultval)
    ...
    public static double getIC2Ratio() {
        return Config.getConfig()
            .get(Configuration.CATEGORY_GENERAL, "Energy per IC2 EU", 0.4)
            .getDouble(0.4);
    }
    ...
    public static double getRFRatio() {
        return Config.getConfig()
            .get(Configuration.CATEGORY_GENERAL, "Energy per RF", 0.1)
            .getDouble(0.1);
    }
    ...
    public static double getAE2Ratio() {
        return Config.getConfig()
            .get(Configuration.CATEGORY_GENERAL, "Energy per AE", 0.2)
            .getDouble(0.2);
    }
    ...
    public static void registerModSpecificModules() {
        ...
        if (isCOFHLibLoaded() || isCOFHCoreLoaded()) {
            ModuleManager.addModule(
                new OmniWrenchModule(Collections.singletonList((IModularItem) MPSItems.getInstance().powerTool)));
        }

        // Thaumcraft
        if (isThaumCraftLoaded() && enableThaumGogglesModule()) {
            ModuleManager.addModule(
                new ThaumGogglesModule(
                    Collections.singletonList((IModularItem) MPSItems.getInstance().powerArmorHead)));
        }
        ...
        if (isIndustrialCraftLoaded()) {
            ModuleManager.addModule(
                new HazmatModule(
                    Arrays.<IModularItem>asList(
                        MPSItems.getInstance().powerArmorHead,
                        MPSItems.getInstance().powerArmorTorso,
                        MPSItems.getInstance().powerArmorLegs,
                        MPSItems.getInstance().powerArmorFeet)));
        }

        // Galacticraft
        if (isGalacticraftLoaded()) {
            ModuleManager.addModule(
                new AirtightSealModule(
                    Collections.singletonList((IModularItem) MPSItems.getInstance().powerArmorHead)));
        }
        ...
        if (isForestryLoaded()) {
            ModuleManager.addModule(
                new GrafterModule(Collections.singletonList((IModularItem) MPSItems.getInstance().powerTool)));
            ModuleManager
                .addModule(new ScoopModule(Collections.singletonList((IModularItem) MPSItems.getInstance().powerTool)));
            ModuleManager.addModule(
                new ApiaristArmorModule(
                    Arrays.<IModularItem>asList(
                        MPSItems.getInstance().powerArmorHead,
                        MPSItems.getInstance().powerArmorTorso,
                        MPSItems.getInstance().powerArmorLegs,
        ...
        if (isChiselLoaded()) {
            try {
                ModuleManager.addModule(
                    new ChiselModule(Collections.singletonList((IModularItem) MPSItems.getInstance().powerTool)));
            } catch (Exception e) {
        ...
        if (isAppengLoaded()) {
            ModuleManager.addModule(
                new AppEngWirelessModule(Collections.singletonList((IModularItem) MPSItems.getInstance().powerTool)));

            // Extra Cells 2
            if (isExtraCellsLoaded()) ModuleManager.addModule(
                new AppEngWirelessFluidModule(
                    Collections.singletonList((IModularItem) MPSItems.getInstance().powerTool)));
        }

        // Multi-Mod Compatible OmniProbe
        if (isEnderIOLoaded() || isMFRLoaded() || isRailcraftLoaded()) {
            ModuleManager.addModule(
                new OmniProbeModule(Collections.singletonList((IModularItem) MPSItems.getInstance().powerTool)));
        }

        // Compact Machines Personal Shrinking Device
        if (isCompactMachinesLoaded()) {
            ModuleManager.addModule(
                new PersonalShrinkingModule(
                    Collections.singletonList((IModularItem) MPSItems.getInstance().powerTool)));
        }
    }
}
