...
public class RenderEventHandler {
    ...
    private final DrawableMuseRect frame = new DrawableMuseRect(
        Config.keybindHUDx(),
        Config.keybindHUDy(),
        Config.keybindHUDx() + (double) 16,
        Config.keybindHUDy() + (double) 16,
        true,
    ...
    public void onPreRenderPlayer(RenderPlayerEvent.Pre event) {
        if (!event.entityPlayer.capabilities.isFlying && !event.entityPlayer.onGround
            && this.playerHasFlightOn(event.entityPlayer)) {
        ...
        if (Config.keybindHUDon()) {
            ...
            for (ClickableKeybinding kb : KeybindManager.getKeybindings()) {
                if (kb.displayOnHUD) {
                    ...
                    frame.setWidth(
                        stringwidth + kb.getBoundModules()
                            .size() * 16);
                    frame.draw();
