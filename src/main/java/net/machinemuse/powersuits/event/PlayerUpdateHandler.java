...
public class PlayerUpdateHandler {
    ...
    public void onPlayerUpdate(LivingEvent.LivingUpdateEvent e) {
        if (e.entity instanceof EntityPlayer) {
            ...
            if (foundItem) {
                player.fallDistance = (float) MovementManager
                    .computeFallHeightFromVelocity(MuseMathUtils.clampDouble(player.motionY, -1000.0, 0.0));
                ...
                if (currHeat > maxHeat) {
                    player.attackEntityFrom(
                        MuseHeatUtils.overheatDamage,
                        (float) (Math.sqrt(currHeat - maxHeat)/* was (int) */ / 4));
                    player.setFire(1);
                } else {
                    player.extinguish();
                }
