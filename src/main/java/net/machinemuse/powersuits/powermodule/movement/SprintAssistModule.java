...
public class SprintAssistModule extends PowerModuleBase implements IToggleableModule, IPlayerTickModule {
    ...
    public void onPlayerTickActive(EntityPlayer player, ItemStack item) {
        ...
        if (player.isSprinting()) {
            ...
            if (sprintCost < totalEnergy) {
                ...
                player.getFoodStats()
                    .addExhaustion((float) (-0.01 * exhaustion * exhaustionComp));
    ...
    public void onPlayerTickInactive(EntityPlayer player, ItemStack item) {
        if (item != null) {
            NBTTagList modifiers = item.getTagCompound()
                .getTagList("AttributeModifiers", (byte) 10);
    ...
    public void setMovementModifier(ItemStack item, double multiplier) {
        NBTTagList modifiers = item.getTagCompound()
            .getTagList("AttributeModifiers", (byte) 10); // Type 10 for tag compound
        item.getTagCompound()
            .setTag("AttributeModifiers", modifiers);
        ...
        if (sprintModifiers.hasNoTags()) modifiers.appendTag(
            new AttributeModifier(1, TAGUUID, multiplier - 1, "generic.movementSpeed", "Sprint Assist").toNBT());
    }
