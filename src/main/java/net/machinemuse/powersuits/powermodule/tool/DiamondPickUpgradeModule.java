...
public class DiamondPickUpgradeModule extends PowerModuleBase implements IBlockBreakingModule, IToggleableModule {
        ...
        if (!Items.iron_pickaxe.canHarvestBlock(block, stack)) {
            if (Items.diamond_pickaxe.canHarvestBlock(block, stack)) {
                return ElectricItemUtils.getPlayerEnergy(player)
                    > ModuleManager.computeModularProperty(stack, PickaxeModule.PICKAXE_ENERGY_CONSUMPTION);
            }
        }
        return false;
    }
    ...
    public void handleBreakSpeed(BreakSpeed event) {
        event.newSpeed = (float) ModuleManager
            .computeModularProperty(event.entityPlayer.getCurrentEquippedItem(), PickaxeModule.PICKAXE_HARVEST_SPEED);
    }
}
