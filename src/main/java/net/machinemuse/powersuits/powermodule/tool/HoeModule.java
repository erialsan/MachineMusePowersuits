            ...
            for (int i = (int) Math.floor(-radius); i < radius; i++) {
                for (int j = (int) Math.floor(-radius); j < radius; j++) {
                    if (i * i + j * j < radius * radius) {
                        ...
                        if (side != 0 && world.getBlock(newX, y + 1, newZ)
                            .isAir(world, newX, y + 1, newZ) && (block == Blocks.grass || block == Blocks.dirt)) {
                            world.setBlock(newX, y, newZ, Blocks.farmland);
                            ElectricItemUtils.drainPlayerEnergy(
                                player,
                                ModuleManager.computeModularProperty(itemStack, HOE_ENERGY_CONSUMPTION));
                            used = true;
                        }
                    }
                }
