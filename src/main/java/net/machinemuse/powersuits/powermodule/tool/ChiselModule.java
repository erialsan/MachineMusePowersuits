...
public class ChiselModule extends PowerModuleBase implements IBlockBreakingModule, IToggleableModule {
        ...
        if (ModCompatibility.isChiselLoaded() && ITEM_CHISEL != null) {
            if (ForgeHooks.canToolHarvestBlock(block, meta, new ItemStack(ITEM_CHISEL))) {
                return ElectricItemUtils.getPlayerEnergy(player)
                    > ModuleManager.computeModularProperty(stack, CHISEL_ENERGY_CONSUMPTION);
            }
        }
        return false;
    }
    ...
    public boolean onBlockDestroyed(ItemStack stack, World world, Block block, int x, int y, int z,
        ...
        if (canHarvestBlock(stack, block, meta, player)) {
            ElectricItemUtils
                .drainPlayerEnergy(player, ModuleManager.computeModularProperty(stack, CHISEL_ENERGY_CONSUMPTION));
            return true;
        } else {
            return false;
        }
    ...
    public void handleBreakSpeed(BreakSpeed event) {
        event.newSpeed *= ModuleManager
            .computeModularProperty(event.entityPlayer.getCurrentEquippedItem(), CHISEL_HARVEST_SPEED);
    }
}
