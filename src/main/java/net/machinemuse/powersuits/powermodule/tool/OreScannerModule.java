...
public class OreScannerModule extends PowerModuleBase implements IRightClickModule {
    ...
    public PowerModuleBase addIntTradeoffProperty(String tradeoffName, String propertyName, double multiplier,
        String unit, int roundTo, int offset) {
        units.put(propertyName, unit);
        return addPropertyModifier(
            propertyName,
            new PropertyModifierIntLinearAdditive(tradeoffName, multiplier, roundTo, offset));
    }

    public void searchForValuables(ItemStack itemStack, EntityPlayer player, World world, int x, int y, int z,
        int side) {
            ...
            for (int sY = cY - yRadius; sY <= cY + yRadius; sY++) {
                for (int sZ = cZ - zRadius; sZ <= cZ + zRadius; sZ++) {
                    ...
                    ElectricItemUtils.drainPlayerEnergy(
                        player,
                        ModuleManager.computeModularProperty(itemStack, ORE_SCANNER_ENERGY_CONSUMPTION));
                ...
                if (Config.useAdvancedOreScannerMessage()) {
                    player.addChatMessage(
                        new ChatComponentText(
                            "[Ore Scanner] Total ore value: " + totalValue
                                + " --- Most valuable: "
                                + highestValue
                                + "\nSearch radius: "
                                + (2 * (int) ModuleManager.computeModularProperty(itemStack, ORE_SCANNER_RADIUS_X) + 1)
                                + "x"
                                + (2 * (int) ModuleManager.computeModularProperty(itemStack, ORE_SCANNER_RADIUS_Y) + 1)
                                + "x"
                    ...
                    player.addChatMessage(
                        new ChatComponentText(
                            "[Ore Scanner] Total ore value: " + totalValue + " --- Most valuable: " + highestValue));
                }
            }
        }
    }
    ...
    public static int getValue(Block blockID, int meta) {
        if ((oreMap.containsKey(Arrays.asList(blockID, meta)))
            && (valueMap.containsKey(oreMap.get(Arrays.asList(blockID, meta))))) {
