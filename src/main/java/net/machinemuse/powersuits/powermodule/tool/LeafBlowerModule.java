...
public class LeafBlowerModule extends PowerModuleBase implements IRightClickModule {
    ...
    private boolean blockCheckAndHarvest(String blocktype, EntityPlayer player, World world, int x, int y, int z) {
        ...
        // Plants
        if ((Objects.equals(blocktype, "plants")) && (block instanceof BlockBush)
        ...
        // Leaves
        if ((Objects.equals(blocktype, "leaves")) && block instanceof BlockLeaves
            && block.canHarvestBlock(player, meta)) {
            block.harvestBlock(world, player, x, y, z, meta);
            world.setBlockToAir(x, y, z);
            return true;
    ...
    private void useBlower(int radius, String blocktypename, ItemStack itemStack, EntityPlayer player, World world,
        int x, int y, int z) {
        int totalEnergyDrain = 0;
            ...
            for (int j = -radius; j < radius; j++) {
                for (int k = -radius; k < radius; k++) {
                    if (blockCheckAndHarvest(blocktypename, player, world, x + i, y + j, z + k)) {
                        totalEnergyDrain += ModuleManager
                            .computeModularProperty(itemStack, LEAF_BLOWER_ENERGY_CONSUMPTION);
                    }
                }
            }
        }
