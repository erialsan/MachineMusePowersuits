...
public class ScoopModule extends PowerModuleBase implements IBlockBreakingModule {
    ...
    public boolean canHarvestBlock(ItemStack stack, Block block, int meta, EntityPlayer player) {
        if (ForgeHooks.canToolHarvestBlock(block, meta, scoop)) {
            return ElectricItemUtils.getPlayerEnergy(player)
                > ModuleManager.computeModularProperty(stack, SCOOP_ENERGY_CONSUMPTION);
        }
        return false;
    }
    ...
    public boolean onBlockDestroyed(ItemStack stack, World world, Block block, int x, int y, int z,
        ...
        if (canHarvestBlock(stack, block, meta, player)) {
            ElectricItemUtils
                .drainPlayerEnergy(player, ModuleManager.computeModularProperty(stack, SCOOP_ENERGY_CONSUMPTION));
            return true;
        } else {
            return false;
        }
    ...
    public void handleBreakSpeed(BreakSpeed event) {
        event.newSpeed *= ModuleManager
            .computeModularProperty(event.entityPlayer.getCurrentEquippedItem(), SCOOP_HARVEST_SPEED);
    }
}
