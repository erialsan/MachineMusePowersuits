...
public class OmniProbeModule extends PowerModuleBase implements IRightClickModule, IPlayerTickModule {
    ...
    public boolean onItemUseFirst(ItemStack itemStack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (Loader.isModLoaded("MineFactoryReloaded")) {
            if (block == Block.getIdFromBlock(GameRegistry.findBlock("MineFactoryReloaded", "cable.redstone")))
                return rednetMeter.getItem()
                    .onItemUseFirst(itemStack, player, world, x, y, z, side, hitX, hitY, hitZ);
        }
        ...
        if (Loader.isModLoaded("Railcraft")) {
            if ((block == Block.getIdFromBlock(GameRegistry.findBlock("Railcraft", "tile.railcraft.machine.alpha")))
                ...
                || (block
                    == Block.getIdFromBlock(GameRegistry.findBlock("Railcraft", "tile.railcraft.machine.epsilon")))
                || (block
                    == Block.getIdFromBlock(GameRegistry.findBlock("Railcraft", "tile.railcraft.machine.delta")))) {
                return rcMeter.getItem()
                    .onItemUseFirst(itemStack, player, world, x, y, z, side, hitX, hitY, hitZ);
            }
        }
    ...
    public void onPlayerTickInactive(EntityPlayer player, ItemStack item) {
        if ((MuseItemUtils.getEIONoCompete(item) != null) && (!MuseItemUtils.getEIONoCompete(item)
            .isEmpty())) {
            if (MuseItemUtils.getEIONoCompete(item)
                .equals(MODULE_OMNIPROBE)) {
