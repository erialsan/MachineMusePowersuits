...
public class ShovelModule extends PowerModuleBase implements IBlockBreakingModule, IToggleableModule {
    ...
    private boolean istEffectiveHarvestTool(Block block, int metadata) {
        ...
        if (emulatedTool.getItem()
            .canHarvestBlock(block, emulatedTool)) return true;

        String effectiveTool = block.getHarvestTool(metadata);
        // some blocks like stairs do no not have a tool assigned to them
        if (effectiveTool == null) {
            if (emulatedTool.func_150997_a/* getStrVsBlock */(block)
                >= ((ItemTool) emulatedTool.getItem()).func_150913_i/* getToolMaterial */()
    ...
    public boolean canHarvestBlock(ItemStack stack, Block block, int meta, EntityPlayer player) {
        if (istEffectiveHarvestTool(block, meta)) {
            return ElectricItemUtils.getPlayerEnergy(player)
                > ModuleManager.computeModularProperty(stack, SHOVEL_ENERGY_CONSUMPTION);
        }
        return false;
    }
    ...
    public boolean onBlockDestroyed(ItemStack stack, World world, Block block, int x, int y, int z,
        ...
        if (canHarvestBlock(stack, block, meta, player)) {
            ElectricItemUtils
                .drainPlayerEnergy(player, ModuleManager.computeModularProperty(stack, SHOVEL_ENERGY_CONSUMPTION));
            return true;
        }
        return false;
    }
    ...
    public void handleBreakSpeed(BreakSpeed event) {
        event.newSpeed *= ModuleManager
            .computeModularProperty(event.entityPlayer.getCurrentEquippedItem(), SHOVEL_HARVEST_SPEED);
    }
}
