...
@Optional.InterfaceList({
    ...
    @Optional.Interface(
        iface = "appeng.api.features.IWirelessTermHandler",
...
public class TerminalHandler implements IWirelessTermHandler, IWirelessFluidTermHandler {
    ...
    public boolean canHandle(ItemStack is) {
        ...
        if (is.getUnlocalizedName() == null) return false;
        return is.getUnlocalizedName()
            .equals("item.powerFist");
    }
    ...
    public boolean usePower(EntityPlayer entityPlayer, double v, ItemStack itemStack) {
        boolean ret = false;
        if ((v * ModCompatibility.getAE2Ratio())
            < (ElectricItemUtils.getPlayerEnergy(entityPlayer) * ModCompatibility.getAE2Ratio())) {
    ...
    public boolean hasPower(EntityPlayer entityPlayer, double v, ItemStack itemStack) {
        return ((v * ModCompatibility.getAE2Ratio())
            < (ElectricItemUtils.getPlayerEnergy(entityPlayer) * ModCompatibility.getAE2Ratio()));
    }
    ...
    private static void registerAEHandler(TerminalHandler handler) {
        AEApi.instance()
            .registries()
            .wireless()
            .registerWirelessHandler(handler);
    }
