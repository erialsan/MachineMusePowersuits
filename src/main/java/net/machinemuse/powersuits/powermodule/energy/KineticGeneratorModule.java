    ...
    public void onPlayerTickActive(EntityPlayer player, ItemStack item) {
        if (!player.isAirBorne) {
            ...
            double distance = Math.sqrt(
                (tag.getInteger("x") - (int) player.posX) * (tag.getInteger("x") - (int) player.posX)
            ...
            if (distance >= 5.0) {
                ...
                if (player.isSprinting()) {
                    ElectricItemUtils.givePlayerEnergy(
                        player,
                        ModuleManager.computeModularProperty(item, KINETIC_ENERGY_GENERATION));
                    MuseHeatUtils
                        .heatPlayer(player, ModuleManager.computeModularProperty(item, KINETIC_HEAT_GENERATION));
                } else {
                    ElectricItemUtils.givePlayerEnergy(
                        player,
                        ModuleManager.computeModularProperty(item, KINETIC_ENERGY_GENERATION) / 2);
                    MuseHeatUtils
                        .heatPlayer(player, ModuleManager.computeModularProperty(item, KINETIC_HEAT_GENERATION) / 2);
                }
            }
        }
    }
