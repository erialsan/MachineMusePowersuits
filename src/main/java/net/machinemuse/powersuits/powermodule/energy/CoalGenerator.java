...
public class CoalGenerator extends PowerModuleBase implements IPlayerTickModule, IToggleableModule {
    ...
    public void onPlayerTickActive(EntityPlayer player, ItemStack item) {
        ...
        int coalNeeded = (int) ModuleManager.computeModularProperty(item, MAX_COAL_STORAGE)
            - MuseItemUtils.getCoalLevel(item);
        if (coalNeeded > 0) {
            for (int i = 0; i < inv.getSizeInventory(); i++) {
                ...
                if (stack != null && stack.getItem() == Items.coal) {
                    ...
                    if (ModuleManager.computeModularProperty(item, MAX_COAL_STORAGE) - MuseItemUtils.getCoalLevel(item)
                        < 1) {
