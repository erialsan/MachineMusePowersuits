...
public class AutoFeederModule extends PowerModuleBase implements IToggleableModule, IPlayerTickModule {
    ...
    public void onPlayerTickActive(EntityPlayer player, ItemStack item) {
        if (Config.useOldAutoFeeder()) {
            ...
            if (foodNeeded > 0 && (eatingEnergyConsumption * foodNeeded < ElectricItemUtils.getPlayerEnergy(player))
                && MuseItemUtils.getFoodLevel(item) > foodNeeded) {
            ...
            double eatingEnergyConsumption = foodNeeded
                * ModuleManager.computeModularProperty(item, EATING_ENERGY_CONSUMPTION);
