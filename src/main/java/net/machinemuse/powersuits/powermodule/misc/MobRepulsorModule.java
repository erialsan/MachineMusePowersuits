...
public class MobRepulsorModule extends PowerModuleBase implements IPlayerTickModule, IToggleableModule {
    ...
    public void onPlayerTickActive(EntityPlayer player, ItemStack item) {
        if (ElectricItemUtils.getPlayerEnergy(player)
            > ModuleManager.computeModularProperty(item, MOB_REPULSOR_ENERGY_CONSUMPTION)) {
    ...
    public void repulse(World world, int i, int j, int k) {
        ...
        List list = world.getEntitiesWithinAABB(
            EntityMob.class,
            AxisAlignedBB
                .getBoundingBox(i - distance, j - distance, k - distance, i + distance, j + distance, k + distance));
