...
public class EntityPlasmaBolt extends EntityThrowable {
    ...
    public EntityPlasmaBolt(World world, EntityLivingBase shootingEntity, double explosiveness, double damagingness,
        int chargeTicks) {
        super(world);
        ...
        Vec3 direction = shootingEntity.getLookVec()
            .normalize();
        double scale = 1.0;
        ...
        this.posX = shootingEntity.posX + direction.xCoord * xoffset
            - direction.yCoord * horzx * yoffset
            - horzz * zoffset;
        this.posY = shootingEntity.posY + shootingEntity.getEyeHeight()
            + direction.yCoord * xoffset
            + (1 - Math.abs(direction.yCoord)) * yoffset;
        this.posZ = shootingEntity.posZ + direction.zCoord * xoffset
            - direction.yCoord * horzz * yoffset
            + horzx * zoffset;
        ...
        if (this.isInWater()) {
            ...
            for (int var3 = 0; var3 < this.size; ++var3) {
                this.worldObj.spawnParticle(
                    "flame",
                    this.posX + Math.random() * 1,
                    this.posY + Math.random() * 1,
                    this.posZ + Math.random() * 0.1,
                    0.0D,
                    0.0D,
                    0.0D);
            }
    ...
    protected void onImpact(MovingObjectPosition event) {
        ...
        switch (event.typeOfHit) {
            case ENTITY:
                if (event.entityHit != null && event.entityHit != shootingEntity) {
                    event.entityHit
                        .attackEntityFrom(DamageSource.causeThrownDamage(this, this.shootingEntity), (int) damage);
                }
                break;
            case BLOCK:
                break;
        ...
        if (FMLCommonHandler.instance()
            ...
            this.worldObj.createExplosion(
                ...
                this.posY,
                this.posZ,
                (float) (this.size / 50.0f * 3 * this.explosiveness),
                flag);
        }
        for (int var3 = 0; var3 < 8; ++var3) {
            this.worldObj.spawnParticle(
                "flame",
                this.posX + Math.random() * 0.1,
                this.posY + Math.random() * 0.1,
                this.posZ + Math.random() * 0.1,
                0.0D,
                0.0D,
                0.0D);
