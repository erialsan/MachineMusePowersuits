...
/**
 * Custom renderer for the power armor and tools. Note - this only renders the item as held in hand, in an inventory
 * slot, or sitting on the ground.
...
public class ToolRenderer extends MuseRender implements IItemRenderer {
    ...
    /**
     * Called to actually render the item. type is as above, item is the item to render, and data is some extra data
     ...
     * <p/>
     * INVENTORY_BLOCK - Determines if the item should be rendered in GUI inventory slots as a 3D block or as a 2D
     * texture.
     */
    @Override
    ...
    public void renderFirstPersonArm(EntityClientPlayerMP entityclientplayermp, float par1) {
        ...
        GL11.glTranslatef(
            -swingProgressy * 0.3F,
            MathHelper.sin(MathHelper.sqrt_float(swingProgress) * (float) Math.PI * 2.0F) * 0.4F,
            -swingProgressx * 0.4F);
        ...
        // GL11.glBindTexture(GL11.GL_TEXTURE_2D, mc.renderEngine.getTextureForDownloadableImage(mc.thePlayer.skinUrl,
