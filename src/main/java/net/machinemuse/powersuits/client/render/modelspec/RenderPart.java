...
public class RenderPart extends ModelRenderer {
    ...
    public void render(float scale) {
        ...
        for (NBTTagCompound nbt : NBTTagAccessor.getValues(renderSpec)) {
            ModelPartSpec part = ModelRegistry.getInstance()
                .getPart(nbt);
            /* checks for None TODO: Null check for Java port. */
            if (part != null) {
                if (part.slot == ((IArmorModel) (ArmorModelInstance.getInstance())).getVisibleSection()
                    && part.morph.apply(ArmorModelInstance.getInstance()) == parent) {
