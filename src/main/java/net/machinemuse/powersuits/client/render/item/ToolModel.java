...
public class ToolModel extends ModelBase {
    ...
    public void render(Entity entity, float scale, boolean renderTypeIsFirstPerson, Colour c1, Colour glow) {
        ...
        if (!tap) {
            ...
            if (!Keyboard.isKeyDown(Keyboard.KEY_NUMPAD0) && !Keyboard.isKeyDown(Keyboard.KEY_NUMPAD1)
     ...
     */
    public void setPose(float indexOpen, float indexFlex, float thumbOpen, float thumbFlex, float otherFingersOpen,
        float otherFingersFlex) {
    ...
    public void setPoseForPlayer(EntityPlayer player, ItemStack itemStack) {
        if (player.isUsingItem() && player.getCurrentEquippedItem() != null
            && ModuleManager
                .itemHasActiveModule(player.getCurrentEquippedItem(), PlasmaCannonModule.MODULE_PLASMA_CANNON)) {
