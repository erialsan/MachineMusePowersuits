 ...
 *
 * FIXME: IMPORTANT!!!!: Note that SmartMoving will mess up the rendering here and the armor's yaw will not change with
 * the player's yaw but will be fine with it not installed.
 *
 */
public class VanillaArmorModel extends ModelBiped implements IArmorModel {
    ...
    public static VanillaArmorModel getInstance() {
        if (INSTANCE == null) INSTANCE = new VanillaArmorModel();
        return INSTANCE;
    }
