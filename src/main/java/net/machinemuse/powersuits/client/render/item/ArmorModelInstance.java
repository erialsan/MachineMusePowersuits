        ...
        if (instance == null) {
            if (ModCompatibility.isRenderPlayerAPILoaded() && ModCompatibility.isSmartRendererLoaded()) {
                try {
                    ...
                    instance = Class.forName("net.machinemuse.powersuits.client.render.item.SMovingArmorModel")
                        .asSubclass(ModelBiped.class)
                        .newInstance();
                    MuseLogger.logInfo("SmartMoving armor model loaded successfully!");
                } catch (Exception e) {
                    MuseLogger.logInfo(
                        "Smart Moving armor model did not load successfully. Either Smart Moving is not installed, or there was another problem.");
