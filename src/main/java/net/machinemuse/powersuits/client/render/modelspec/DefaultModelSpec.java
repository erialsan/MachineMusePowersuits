...
public class DefaultModelSpec {
    ...
    public static ModelSpec loadModel(ResourceLocation file, String[] textures) {
        WavefrontObject model = ModelRegistry.getInstance()
            .loadModel(file);
        if (model != null) {
            return (ModelRegistry.getInstance()
                .put(MuseStringUtils.extractName(file), new ModelSpec(model, textures, null, null, file.toString())));
        }
        return null;
    }

    public static void makeEntries(MorphTarget target, int slot, int colourIndex, Boolean glow, String[] names,
        ModelSpec model) {
        ...
        switch (slot) {
            case 0:
                list.addAll(
                    makePrefs(
                        "mps_helm",
                        "helm_main;helm_tube_entry1;helm_tubes;helm_tube_entry2".split(";"),
                        0,
                        false));
            ...
            case 1:
                ...
                list.addAll(
                    makePrefs(
                        "mps_chest",
                        "belt;chest_main;polySurface36;backpack;chest_padding".split(";"),
                        0,
                        false));
    ...
    public static List<NBTTagCompound> makePrefs(String modelname, String[] partnames, int colour, boolean glow) {
        ...
        ModelSpec model = ModelRegistry.getInstance()
            .get(modelname);
