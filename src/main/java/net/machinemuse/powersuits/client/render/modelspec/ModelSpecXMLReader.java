...
public class ModelSpecXMLReader {
    ...
    public static ModelSpecXMLReader getINSTANCE() {
        if (INSTANCE == null) INSTANCE = new ModelSpecXMLReader();
        return INSTANCE;
    }
    ...
    public void parseModel(Node modelnode) {
        ...
        if (modelnode.getNodeType() == Node.ELEMENT_NODE) {
            ...
            textures = eElement.getAttribute("textures")
                .split(",");
            ...
            WavefrontObject model = ModelRegistry.getInstance()
                .loadModel(new ResourceLocation(file));
            if (model != null) {
                ModelSpec modelspec = new ModelSpec(model, textures, offset, rotation, file);
                ModelSpec existingspec = ModelRegistry.getInstance()
                    .put(MuseStringUtils.extractName(file), modelspec);
    ...
    public void parseParts(Node partNode, ModelSpec modelspec, int slot, MorphTarget target) {
        if (partNode.getNodeType() == Node.ELEMENT_NODE) {
            ...
            if (polygroup != null) {
                ModelPartSpec partspec = new ModelPartSpec(
                    modelspec,
                    target,
                    polygroup,
                    slot,
                    0,
                    (defaultglow != null) ? defaultglow : false,
                    name);
