...
/**
 * Packet for requesting to purchase an upgrade. Player-to-server. Server decides whether it is a valid upgrade or not
 * and replies with an associated
...
public class MusePacketInstallModuleRequest extends MusePacket {
    ...
    public void handleServer(EntityPlayerMP player) {
        ...
        if (moduleName != null) {
            ...
            if ((!ModuleManager.itemHasModule(stack, moduleName)
                && MuseItemUtils.hasInInventory(cost, player.inventory)) || player.capabilities.isCreativeMode) {
    ...
    public static MusePacketInstallModuleRequestPackager getPackagerInstance() {
        if (PACKAGERINSTANCE == null) PACKAGERINSTANCE = new MusePacketInstallModuleRequestPackager();
