...
public class MusePacketSalvageModuleRequest extends MusePacket {
        ...
        if (moduleName != null) {
            ...
            if (ModuleManager.itemHasModule(stack, moduleName)) {
                ...
                for (ItemStack refundItem : refund) {
                    slots.addAll(
                        MuseItemUtils.giveOrDropItemWithChance(refundItem.copy(), player, Config.getSalvageChance()));
                }
                slots.add(itemSlot);
    ...
    public static MusePacketSalvageModuleRequestPackager getPackagerInstance() {
        if (PACKAGERINSTANCE == null) PACKAGERINSTANCE = new MusePacketSalvageModuleRequestPackager();
