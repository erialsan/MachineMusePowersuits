...
public class MusePacketPropertyModifierConfig extends MusePacket {
    ...
    public void write() {
        ...
        for (IPowerModule module : ModuleManager.getAllModules()) {
            ...
            writeInt(
                module.getPropertyModifiers()
                    .size());
            for (Map.Entry<String, List<IPropertyModifier>> entry : module.getPropertyModifiers()
                .entrySet()) {
    ...
    public void handleClient(EntityPlayer player) {
        ...
        for (int i = 0; i < numModules; i++) {
            ...
            if (module instanceof PowerModuleBase) ((PowerModuleBase) module).setIsAllowed(allowed);
            int numProps = d.readInt(data);
            for (int j = 0; j < numProps; j++) {
                String propName = d.readString(data);
                int numModifiers = d.readInt(data);
                List<IPropertyModifier> proplist = module.getPropertyModifiers()
                    .get(propName);
    ...
    public static MusePacketPropertyModifierConfigPackager getPackagerInstance() {
        if (PACKAGERINSTANCE == null) PACKAGERINSTANCE = new MusePacketPropertyModifierConfigPackager();
