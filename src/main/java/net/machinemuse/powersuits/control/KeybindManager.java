...
public class KeybindManager {
    ...
    public static void writeOutKeybinds() {
        ...
        try {
            File file = new File(
                Loader.instance()
                    .getConfigDir() + "/machinemuse/",
                "powersuits-keybinds.cfg");
            if (!file.exists()) {
                file.createNewFile();
            }
            writer = new BufferedWriter(new FileWriter(file));
            List<IPowerModule> modulesToWrite = MuseItemUtils
                .getPlayerInstalledModules(Minecraft.getMinecraft().thePlayer);
            for (ClickableKeybinding keybinding : getInstance().keybindings) {
                writer.write(
                    keybinding.getKeyBinding()
                        .getKeyCode() + ":"
                        + keybinding.getPosition()
                            .x()
                        + ':'
                        + keybinding.getPosition()
                            .y()
                        + ':'
                        + keybinding.displayOnHUD
                        + ':'
                        + keybinding.toggleval
                        + '\n');
            ...
            while (reader.ready()) {
                ...
                if (line.contains(":")) {
                    ...
                    if (!KeyBinding.hash.containsItem(id)) {
                        MusePoint2D position = new MusePoint2D(
                            Double.parseDouble(exploded[1]),
                            Double.parseDouble(exploded[2]));
                        ...
                        workingKeybinding = new ClickableKeybinding(
                            new KeyBinding(Keyboard.getKeyName(id), id, KeybindKeyHandler.mps),
                            position,
                            free,
                            displayOnHUD);
