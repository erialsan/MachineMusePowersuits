 ...
 */
@Optional.InterfaceList({ @Optional.Interface(iface = "mekanism.api.IMekWrench", modid = "Mekanism", striprefs = true),
    ...
    @Optional.Interface(
        ...
        striprefs = true) })
public class ItemPowerFist extends MPSItemElectricTool implements IModularItem, IToolGrafter, IToolHammer, IMFRHammer,
    IToolCrowbar, IAEWrench, IToolWrench, com.bluepowermod.api.misc.IScrewdriver, mrtjp.projectred.api.IScrewdriver,
    ITool, IMekWrench, IModeChangingModularItem {
    ...
    public boolean hitEntity(ItemStack stack, EntityLivingBase entityBeingHit, EntityLivingBase entityDoingHitting) {
        ...
        if (entityDoingHitting instanceof EntityPlayer
            && ModuleManager.itemHasActiveModule(stack, MeleeAssistModule.MODULE_MELEE_ASSIST)) {
            ...
            if (ElectricItemUtils.getPlayerEnergy(player) > drain) {
                ...
                if (entityBeingHit.attackEntityFrom(damageSource, (int) damage)) {
                    ...
                    entityBeingHit.addVelocity(
                        lookVec.xCoord * knockback,
                        Math.abs(lookVec.yCoord + 0.2f) * knockback,
                        lookVec.zCoord * knockback);
                }
            }
        }
        return true;
    ...
    public Multimap<String, AttributeModifier> getAttributeModifiers(ItemStack stack) {
        ...
        multimap.put(
            ...
            new AttributeModifier(
                field_111210_e,
                "Weapon modifier",
                ModuleManager.computeModularProperty(stack, MeleeAssistModule.PUNCH_DAMAGE),
                0));
        return multimap;
    }
    ...
    public ItemStack onItemRightClick(ItemStack itemStack, World world, EntityPlayer player) {
        for (IRightClickModule module : ModuleManager.getRightClickModules()) {
            if (module.isValidForItem(itemStack)
                && ModuleManager.itemHasActiveModule(itemStack, module.getDataName())) {
    ...
    public void onPlayerStoppedUsing(ItemStack itemStack, World world, EntityPlayer player, int par4) {
        ...
        if (module != null) ((IRightClickModule) module).onPlayerStoppedUsing(itemStack, world, player, par4);
    }
    ...
    public boolean onItemUseFirst(ItemStack itemStack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        String mode = getActiveMode(itemStack, player);
        IPowerModule module = ModuleManager.getModule(mode);
        return module instanceof IRightClickModule
    ...
    public boolean canHarvestBlock(ItemStack stack, Block block, int meta, EntityPlayer player) {
        ...
        for (IBlockBreakingModule module : ModuleManager.getBlockBreakingModules()) {
            if (ModuleManager.itemHasActiveModule(stack, module.getDataName())
                && module.canHarvestBlock(stack, block, meta, player)) {
                return true;
            }
        }
        return false;
    ...
    public float getSaplingModifier(ItemStack itemStack, World world, EntityPlayer entityPlayer, int x, int y, int z) {
        if (ModuleManager.itemHasActiveModule(itemStack, GrafterModule.MODULE_GRAFTER)) {
            ElectricItemUtils.drainPlayerEnergy(
                entityPlayer,
                ModuleManager.computeModularProperty(itemStack, GrafterModule.GRAFTER_ENERGY_CONSUMPTION));
            MuseHeatUtils.heatPlayer(
                entityPlayer,
                ModuleManager.computeModularProperty(itemStack, GrafterModule.GRAFTER_HEAT_GENERATION));
            return 100.0f;
        }
        return 0.0f;
    }
    ...
    public boolean isUsable(ItemStack itemStack, EntityLivingBase entityLivingBase, int i, int i1, int i2) {
        return entityLivingBase instanceof EntityPlayer && this.getActiveMode(itemStack)
            .equals(OmniWrenchModule.MODULE_OMNI_WRENCH);
    }
    ...
    public void setActiveMode(ItemStack itemStack, String newMode) {
        NuminaItemUtils.getTagCompound(itemStack)
            .setString("mode", newMode);
    }
    ...
    public IIcon getModeIcon(String mode, ItemStack itemStack, EntityPlayer player) {
        if (!mode.isEmpty()) return ModuleManager.getModule(mode)
            .getIcon(itemStack);
        return null;
    }
    ...
    public String getActiveMode(ItemStack itemStack) {
        String modeFromNBT = NuminaItemUtils.getTagCompound(itemStack)
            .getString("mode");
