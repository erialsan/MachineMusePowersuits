...
public abstract class ItemPowerArmor extends ItemElectricArmor implements ISpecialArmor, IApiaristArmor {
    ...
    public Multimap getAttributeModifiers(ItemStack stack) {
        ...
        parent.put(
            ...
            new AttributeModifier(
                UUID.fromString("448ef0e9-9b7c-4e56-bf3a-6b52aeabff8d"),
                "generic.knockbackResistance",
                0.25,
                0));
        return parent;
    ...
    public boolean hasColor(ItemStack stack) {
        ...
        return ModuleManager.tagHasModule(itemTag, TintModule.RED_TINT)
            || ModuleManager.tagHasModule(itemTag, TintModule.GREEN_TINT)
            || ModuleManager.tagHasModule(itemTag, TintModule.BLUE_TINT);
    }
    ...
    public void damageArmor(EntityLivingBase entity, ItemStack stack, DamageSource source, int damage, int slot) {
        if (entity instanceof EntityPlayer) {
            if (source == MuseHeatUtils.overheatDamage) {} else if (source.isFireDamage()) {
                EntityPlayer player = (EntityPlayer) entity;
                if (!source.equals(DamageSource.onFire)
                    || MuseHeatUtils.getPlayerHeat(player) < MuseHeatUtils.getMaxHeat(player)) {
                    MuseHeatUtils.heatPlayer(player, damage);
                }
            } else {
                double enerConsum = ModuleManager
                    .computeModularProperty(stack, MuseCommonStrings.ARMOR_ENERGY_CONSUMPTION);
    ...
    public boolean protectPlayer(EntityPlayer player, ItemStack armor, String cause, boolean doProtect) {
        ...
        // ElectricItemUtils.drainPlayerEnergy(player, ModuleManager.computeModularProperty(armor,
        // ApiaristArmorModule.APIARIST_ARMOR_ENERGY_CONSUMPTION));
        // return true;
        // }
        return false;
    }
    ...
    public boolean protectEntity(EntityLivingBase player, ItemStack armor, String cause, boolean doProtect) {
        if (ModuleManager.itemHasActiveModule(armor, ApiaristArmorModule.MODULE_APIARIST_ARMOR)) {
            ElectricItemUtils.drainPlayerEnergy(
                (EntityPlayer) player,
                ModuleManager.computeModularProperty(armor, ApiaristArmorModule.APIARIST_ARMOR_ENERGY_CONSUMPTION));
            return true;
        }
        return false;
    }
